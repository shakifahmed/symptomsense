...
export interface SuggestInput {
  ...
  /** 1 = single question (default), 2-5 = batch mode for Likert-scale UI */
  batch_size?: number;
  /** Number of pool questions already asked in the current round (excludes seeds and variant forms) */
...
export interface SuggestOutput {
  ...
  age_group: string;
  /** Mutually exclusive variant options — show as selection instead of yes/no */
  options: string[] | null;
  /** Batch of questions for Likert-scale UI (populated when batch_size > 1) */
  batch: BatchQuestion[] | null;
}
